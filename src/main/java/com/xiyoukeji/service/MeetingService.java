...
public class MeetingService {
    ...
    public List<Object[]> getProjectListFromMeeting() {
        String sql = "SELECT project.id,project.project_name,project.province_name,project.city_name,project.project_resource,project.project_stage,user.name as user_name,foundation.name as foundation_name ,evaluateAvg.item_all,project.project_schedule from project JOIN meeting_project ON project.id = meeting_project.project_id JOIN meeting ON meeting_project.meeting_id = meeting.id JOIN user_project ON project.id = user_project.project_id JOIN user ON user.id = user_project.user_id LEFT OUTER JOIN project_foundation ON project.id = project_foundation.project_id LEFT OUTER JOIN foundation ON foundation.id = project_foundation.foundation_id LEFT OUTER JOIN project_evaluateAvg ON project.id = project_evaluateAvg.project_id LEFT OUTER JOIN evaluateAvg ON evaluateAvg.id = project_evaluateAvg.evaluateAvg_id WHERE  (project.false_del = 0 OR project.false_del is null) GROUP by project.id ORDER by project.sequence";
